...
# === Define custom generate function for vLLM ===
def generate(model, input_ids, past_key_values, max_new_tokens):
    sampling_params = SamplingParams(max_tokens=max_new_tokens, temperature=0, n=1)  # 修正 n=1
    prompt = model.get_tokenizer().decode(input_ids[0], skip_special_tokens=True)
    prompts = [prompt] * 1
    with autocast():
        outputs = model.generate(prompts, sampling_params)
    return model.get_tokenizer().encode(outputs[0].outputs[0].text, return_tensors="pt").to(input_ids.device)
...
def main():
    ...
    # use HF load model for test PPL
    ...
    for i in tqdm(range(5), desc="Warm Up..."):
        ...
        generated = generate(vllm, input_ids, past_key_values, max_new_tokens)
